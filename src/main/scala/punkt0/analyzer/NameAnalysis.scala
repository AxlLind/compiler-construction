...
object NameAnalysis extends Phase[Program, Program] {
  ...
  def attachAndValidateType(scope: Scope, t: TypeTree) = t match {
    case id: Identifier =>
      attachSymbols(scope, id)
      id.maybeSymbol.foreach(_ match {
        case c: ClassSymbol => {}
        case _ => Reporter.error("Given identifier is not a type.", t)
      })
    case _ => {}
  }
  ...
  def attachSymbols(scope: Scope, tree: Tree): Unit = tree match {
    ...
    case t: MethodDecl =>
      ...
      attachAndValidateType(scope, t.retType)
